...
let template index prop =
  ...
  and
    transitionTrigger = {
      ...
      assignments = [(Property.prop_neg prop)^" = true"];
      comment = "The trigger is pulled. We must ensure that the trigger of the positive literal is not pulled too. We do this by ensuring that at most one of the positive and the negative literal holds during this cycle over each subformula.";
    }
  and
    transitionNoTrigger = {
      select= [];
